...
DCLinear::DCLinear(System& sys, const Forcefield& ff,
  ...
  data(sys, ff, set)
{
  atomSize = kind.NumAtoms();

  idExchange = new DCRotateCOM(&data, atomSize);
  //First atom of the molecule
  forward.push_back(new DCSingle(&data, 0));
  backward.push_back(new DCSingle(&data, atomSize - 1));
  //second atom of the molecule
  if(atomSize > 1) {
    forward.push_back(new DCOnSphere(&data, kind, 1, 0));
    backward.push_back(new DCOnSphere(&data, kind, atomSize - 2, atomSize - 1));
  }
}
